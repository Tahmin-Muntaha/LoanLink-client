...
createdAt}=loan
  ...
  const updateMutation=useMutation({
    mutationFn:async(updateData)=>{
        const res=await axios.put(`http://localhost:3000/details/${id}`,updateData)
        return res.data
    },
    onSuccess:()=>{
  ...
  const onSubmit=async(data)=>{
    console.log(data)
    const {description,documents,max,rate,title,showOnHome,category,emis,img}=data
